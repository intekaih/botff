...
def worker(thread_id, proxy_dict=None):
    try:
        ...
        if res and "uid" in res and "password" in res:
            ...
            with file_lock:
                with open(
                    os.path.join(os.path.dirname(__file__), "data", "conbogay.txt"),
                    "a",
                    encoding="utf-8",
                ) as f1:
...
def create_acc(region, proxy=None):
    ...
    response = requests.post(
        url, headers=headers, data=data, proxies=proxy, verify=False
    )
    try:
...
def token(uid, password, region, proxy=None):
    ...
    response = requests.post(
        url, headers=headers, data=body, proxies=proxy, verify=False
    )
...
def chooseregion(data_bytes, jwt_token, proxy=None):
    ...
    response = requests.post(
        url, data=payload, headers=headers, proxies=proxy, verify=False
    )
...
def login(
    ...
    data = payload
    data = data.replace(
        "afcfbf13334be42036e4f742c80b956344bed760ac91b3aff9b607a610ab4390".encode(),
        access_token.encode(),
    )
    ...
    RESPONSE = requests.post(
        URL, headers=headers, data=Final_Payload, proxies=proxy, verify=False
    )
...
def login_server(
    uid, password, access_token, open_id, response, status_code, name, region
):
    lang = get_region(region)
    lang_b = lang.encode("ascii")
...
def GET_LOGIN_DATA(JWT_TOKEN, PAYLOAD, region, proxy=None):
    ...
    while attempt < max_retries:
        try:
            response = requests.post(
                url, headers=headers, data=PAYLOAD, proxies=proxy, verify=False
            )
            ...
            print(
                f"{red}{bold}Request failed: {e}. Attempt {attempt + 1} of {max_retries}. Retrying..."
            )
            attempt += 1
            time.sleep(2)
...
░ ░   ░    ░      ░   ░ ░    ░     ░░   ░   ░   ▒     ░      ░ ░ ░ ▒    ░░   ░
    ...
    if os.path.exists(proxy_path):
        ...
        if proxies_list:
            print(
                f"{Fore.GREEN}[+] Đã tải {len(proxies_list)} Proxy từ {proxy_path}.{Fore.RESET}"
            )
    else:
        print(
            f"{Fore.YELLOW}[-] Không có file Proxy ({proxy_path}). Dùng IP gốc.{Fore.RESET}"
        )
    ...
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        ...
        for i in range(total_acc):
            ...
            futures.append(executor.submit(worker, i + 1, proxy_dict))
